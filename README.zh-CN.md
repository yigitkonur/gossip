...
Gossip 让 **Claude Code** 和 **Codex** 能在同一台机器上协同工作。

用最简单的话说：
- **Claude Code** 负责审阅、规划、质疑方案。
- **Codex** 负责实现和执行。
- **Gossip** 是本地桥接层，负责把它们连接起来。
...
Claude Code 会通过这个命令和 Gossip 进行 MCP 通信。
...
Gossip 通过 TUI 状态机和 thread readiness 来判断这一点。如果系统还没 ready，就会直接拒绝回复，而不是静默丢消息。
...
### 4. Killed sentinel
`gossip kill` 会在状态目录里写一个 sentinel 文件。
...
### 当前实现
- `cmd/gossip/` 中的 Go CLI
- `internal/` 中的 Go 运行时
- `.github/workflows/ci.yml` 中的 Go CI
- `plugins/gossip/` 中的当前插件元数据

### 历史参考
- `ts-legacy/`
- 归档参考脚本
- `ts-legacy/plugins/` 中的归档插件布局
- 仍然描述旧架构的历史文档
